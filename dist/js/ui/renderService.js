...
function renderPaginatedList(container, items, renderItem, emptyMessage, classButton, classDiv, limit = 3) {
    container.innerHTML = '';
    if (items.length === 0) {
...
export function renderStockList(arr, stockList, medications) {
    ...
    renderPaginatedList(stockList, sorted, (med) => createStockReceptionComponent(med), 'Пока ничего нет', 'stock-list__open-card', 'stock-list__card-hidden');
}
