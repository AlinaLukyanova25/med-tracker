...
export function sortByOrderHours(
  arr: Disease[],
...
function hasStock(
  med: MedicationType
): med is Extract<MedicationType, Pill | Capsule | Powder<'Пакетик'>> {
  return (
    (med.type === 'Таблетка' ||
      med.type === 'Капсула' ||
...
): Extract<MedicationType, Pill | Capsule | Powder<'Пакетик'>>[] {
...
export function getActiveDateSet(diseases: Disease[]): Set<string> {
  ...
  for (const dis of diseases) {
    ...
    const firstDay = new Date(
      dis.dateStart.getFullYear(),
      dis.dateStart.getMonth(),
      dis.dateStart.getDate()
    );
    const lastDay = new Date(
      dis.dateEnd.getFullYear(),
      dis.dateEnd.getMonth(),
      dis.dateEnd.getDate()
    );
