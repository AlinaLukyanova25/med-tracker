...
export function renderReceptionList(
  arr: Disease[],
  receptionList: HTMLUListElement,
  ...
  const todayItems = sorted.filter(
    (med) => med.time.getTime() + 900000 - now.getTime() > 0
  );
  const missedItems = sorted.filter(
    (med) => med.time.getTime() + 900000 - now.getTime() <= 0
  );
