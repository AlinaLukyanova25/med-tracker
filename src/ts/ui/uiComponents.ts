...
export function createOpenCardsComponent(
  classStyle: string
): HTMLButtonElement {
...
export function createReceptionMainComponent(
  med: MedicationType,
  today: Date
): string {
  let dosType: string;
...
export function createMissedReceptionComponent(
  med: MedicationType,
  time: Date
): string {
  let dosType: string;
...
export function createStockReceptionComponent(
  med: Extract<MedicationType, Pill | Capsule | Powder<'Пакетик'>>
): string {
  let word: string;
  let verb: string;
  ...
  return `
        <li class="edit__item">
            ...
            <form class="edit__form-item">
                ...
                ${
                  med.type !== 'Аэрозоль' && med.type !== 'Мазь'
                    ? `<div class="edit__input-container edit__dosage"><label for="edit-dosage-${med.medId}">Доза: </label><input type="number" id="edit-dosage-${med.medId}" value="${med.dosage}" min="0" step="0.1"
                ...
                ${
                  ...
                  (med.type === 'Порошок' && med.dosageType === 'Пакетик')
                    ? `<div class="edit__input-container edit__stock"><label for="edit-stock-${med.medId}">Осталось: </label><input type="number" id="edit-stock-${med.medId}" value="${med.stock}" min="1"
                data-property="stock"
                data-object-id="${med.medId}"
                data-typeof-id="string"
                ></div>`
...
export function createEditAddComponent(
  type: SelectMedicationType,
  powderType: string | undefined
): string {
  let html = '';
  switch (type) {
