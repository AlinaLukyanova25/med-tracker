...
export class ModalManager {
  ...
  private modalWarningDescr: HTMLParagraphElement =
    domElements.modal.modalWarningDescr;
  ...
  private modalConfidenceClose: HTMLSpanElement =
    domElements.modal.modalConfidenceClose;
  ...
  init() {
    ...
    this.modalConfidence.addEventListener('click', (e) =>
      this.handleClickConfidence(e)
    );
  ...
  closeModal(target: HTMLElement) {
    if (
      (target.closest('.modal') && !target.closest('.modal__content')) ||
    ...
    if (
      (target.closest('.modal-assign-again') &&
    ...
    if (
      (target.closest('.modal-confidence') &&
  ...
  handleClickConfidence(e: MouseEvent) {
    ...
    if (target.closest('#ok')) {
      ...
      if (
        this.sectionActive.style.display !== 'none' &&
