...
export class CalendarManager {
  ...
  handleCalendarDayClick(e: MouseEvent) {
    ...
    if (
      ...
      this.modal.openModalWarning(
        'Вы ещё не приняли все лекарства на сегодня',
        e,
        cardDay
      );
    } else {
  ...
  setupEventLisneters() {
    ...
    getElement('prev-month', HTMLButtonElement).addEventListener('click', () =>
      this.changeMonth(-1)
    );
    getElement('next-month', HTMLButtonElement).addEventListener('click', () =>
      this.changeMonth(1)
    );
  }
}
