...
export class DiseasesManager {
  ...
  private labelPowderType: HTMLLabelElement =
    domElements.disease.labelPowderType;
  private selectPowderType: HTMLSelectElement =
    domElements.disease.selectPowderType;
  ...
  setupEventListeners() {
    ...
    this.timeLabel.addEventListener('click', (e) =>
      this.handleLabelTimeClick(e)
    );

    this.moreMedButton.addEventListener('click', (e) =>
      this.handleAddMoreMedication(e)
    );

    this.addAgain.addEventListener('submit', (e) =>
      this.handleAgainFormSubmit(e)
    );
    ...
    this.selectPowderType.addEventListener('change', () =>
  ...
  handleAddFormSubmit(e: SubmitEvent) {
    ...
    if (
      (!medArray && this.medArray.length === 0) ||
      medName ||
      dosage ||
      stock ||
      time
  ...
  checkMainPage() {
    ...
    if (
      this.receptionList.querySelectorAll('.reception-list__item').length !==
        0 ||
